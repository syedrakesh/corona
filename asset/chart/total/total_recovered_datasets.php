<?php
	function total_recovered_datasets(){
		function total_recovered_datasets_data(){
			...
			for($i = 1; $i <= $last_id; $i++){
				$sql = "SELECT SUM(per_day_recovered) as per_day_recovered FROM chart WHERE id<=$i order by id asc";
				$q = $pdo14->query($sql);
				$q -> setFetchMode(PDO::FETCH_ASSOC);
				while($row = $q -> fetch()){
		...
		echo "{";
		echo '"label"'.":".'"সর্বমোট সুস্থ্যের সংখ্যা",';
		echo '"data":'.'[';
		echo total_recovered_datasets_data();
		echo '],';
		echo '"fill": false,';
